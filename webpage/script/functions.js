...
function updateMap(color, data) {
  ...
  // update headline
  d3.select("#map_1").select("h2").text(headline + d3.select("#year").node().value);
}
...
function renderBars(color, data) {
  ...
  let axis = d3.select("svg#bars").append("g")
              ...
              .attr("transform", "translate("+ 10 +"," + (svgBarsHeight+margin.top) + ")")
...
function calcColorScale(data) {
  ...
  let scale = d3.scaleQuantile()
              ...
              .range(d3.schemeBlues[(quantiles_calc.length)-1]);
